...
typedef GetFunction = Function<T>(
  String key, [
  bool isSupportedPlatform,
  dynamic androidDefault,
  dynamic iosDefault,
]);
typedef SetFunction = Function<T>(
  String key,
  dynamic value, [
  bool isSupportedPlatform,
]);
...
class GeneralSettings extends BaseSettings {
  ...
  set enabled(bool enabled) => setFunction<bool>(
    SettingsKeys.sdkEnabled,
    enabled,
  );
  ...
  String? get keychainGroupIdentifier => getFunction<String>(
    SettingsKeys.sdkKeychainGroupIdentifier,
    Platform.isIOS == true,
    null,
  );
  set keychainGroupIdentifier(
    String? keychainGroupIdentifier,
  ) => setFunction<String>(
    SettingsKeys.sdkKeychainGroupIdentifier,
  ...
  double get minTokenRefreshInterval => getFunction<double>(
    SettingsKeys.sdkMinTokenRefreshInterval,
  );
  set minTokenRefreshInterval(double minTokenRefreshInterval) => setFunction<
    double
  >(SettingsKeys.sdkMinTokenRefreshInterval, minTokenRefreshInterval);

  bool get shouldDestroySessionOnApiKeyChange => getFunction<bool>(
    SettingsKeys.sdkShouldDestroySessionOnApiKeyChange,
  );
  set shouldDestroySessionOnApiKeyChange(
    bool shouldDestroySessionOnApiKeyChange,
  ) => setFunction<bool>(
    SettingsKeys.sdkShouldDestroySessionOnApiKeyChange,
    minTokenRefreshInterval,
  );
...
class TrackerSettings extends BaseSettings {
  bool get isBackendTimeSyncRequired => getFunction<bool>(
    SettingsKeys.trackerIsBackendTimeSyncRequired,
  );
  set isBackendTimeSyncRequired(bool isBackendTimeSyncRequired) => setFunction<
    bool
  >(SettingsKeys.trackerIsBackendTimeSyncRequired, isBackendTimeSyncRequired);

  int get minBatchSize => getFunction<int>(SettingsKeys.trackerMinBatchSize);
  set minBatchSize(int minBatchSize) => setFunction<int>(
    SettingsKeys.trackerMinBatchSize,
    minBatchSize,
  );

  int get maxBatchSize => getFunction<int>(SettingsKeys.trackerMaxBatchSize);
  set maxBatchSize(int maxBatchSize) => setFunction<int>(
    SettingsKeys.trackerMaxBatchSize,
    maxBatchSize,
  );

  double get autoFlushTimeout => getFunction<double>(
    SettingsKeys.trackerAutoFlushTimeout,
  );
...
class NotificationsSettings extends BaseSettings {
  ...
  bool get encryption => getFunction<bool>(
    SettingsKeys.notificationsEncryption,
  );
  set encryption(bool encryption) => setFunction<bool>(
    SettingsKeys.notificationsEncryption,
    encryption,
  );
...
class InAppMessagingSettings extends BaseSettings {
  double get maxDefinitionUpdateIntervalLimit => getFunction<double>(
    SettingsKeys.inAppMaxDefinitionUpdateIntervalLimit,
  );
  ...
  double get renderingTimeout => getFunction<double>(
    SettingsKeys.inAppMessagingRenderingTimeout,
  );
...
class InjectorSettings extends BaseSettings {
  ...
  set automatic(bool automatic) => setFunction<bool>(
    SettingsKeys.injectorAutomatic,
    automatic,
  );
...
class SettingsImpl extends BaseModule {
  ...
  late NotificationsSettings notifications = NotificationsSettings(
    _getFunction,
    _setFunction,
  );
  late InAppMessagingSettings inAppMessaging = InAppMessagingSettings(
    _getFunction,
    _setFunction,
  );
