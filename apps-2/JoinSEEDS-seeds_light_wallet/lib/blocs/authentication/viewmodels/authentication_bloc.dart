...
class AuthenticationBloc
  ...
  AuthenticationBloc() : super(AuthenticationState.initial()) {
    on<InitAuthStatus>(
        (_, emit) => emit(AuthStatusStateMapper().mapResultToState(state)));
    on<InitOnResumeAuth>(
        (_, emit) => emit(state.copyWith(isOnResumeAuth: true)));
    on<SuccessOnResumeAuth>(
        (_, emit) => emit(state.copyWith(isOnResumeAuth: false)));
    on<UnlockWallet>(
        (_, emit) => emit(state.copyWith(authStatus: AuthStatus.unlocked)));
    on<OnInviteLinkRecived>(
        (_, emit) => emit(state.copyWith(authStatus: AuthStatus.inviteLink)));
    ...
    on<EnablePasscode>(
        (event, _) => settingsStorage.enablePasscode(event.newPasscode));
  ...
  Future<void> _onCreateAccount(
      OnCreateAccount event, Emitter<AuthenticationState> emit) async {
    /// In case there was a recovery in place. We cancel it.
    /// This will clean all data
    await StopRecoveryUseCase().run();
    await SaveAccountUseCase()
        .run(accountName: event.account, authData: event.authData);
    // New account --> re-start auth status
    add(const InitAuthStatus());
  }

  Future<void> _onImportAccount(
      OnImportAccount event, Emitter<AuthenticationState> emit) async {
  ...
  void _onRecoverAccount(
      OnRecoverAccount event, Emitter<AuthenticationState> emit) {
  ...
  void _onSwitchAccount(
      OnSwitchAccount event, Emitter<AuthenticationState> emit) {
  ...
  Future<void> _onLogout(
      OnLogout event, Emitter<AuthenticationState> emit) async {
    // Clear data
